...
tb_pct_cursos AS (
    ...
    FROM 
        tb_usuario_cursos AS t1
    LEFT JOIN 
        tb_cursos_total_eps AS t2
    ON
        t1.descSlugCurso = t2. descSlugCurso
),
...
tb_pct_cursos_pivot AS (
    SELECT
        ...
        SUM(CASE WHEN pctCursoCompleto > 0 AND pctCursoCompleto < 1 THEN 1 ELSE 0 END) AS pctCursoIncompleto,
